...
def display_metrics_dashboard(safe_attempts: int, total_attempts: int, blocked: int, avg_time: float):
    ...
    with col2:
        st.metric("Total Prompts", total_attempts, delta=f"+{total_attempts - blocked}")

    with col3:
        st.metric("Blocked Threats", blocked, delta=f"{(blocked / max(1, total_attempts)) * 100:.1f}% of total")
